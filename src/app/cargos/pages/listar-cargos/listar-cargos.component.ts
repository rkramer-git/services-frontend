...
export class ListarCargosComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.cargoService.atualizarCargoSub$.subscribe(
      (precisaAtualizar)=>{
        if (precisaAtualizar){
  ...
  deletarCargo(carg: Cargo):void{
    const dialogRef = this.dialog.open(ConfirmarDelecaoCargoComponent)

    dialogRef.afterClosed().subscribe(
      (deletar)=>{
        if (deletar){
          this.cargoService.deleteCargo(carg).subscribe(
            ()=>{
              this.snackbar.open('Cargo deletado', 'Ok', {duration: 3000})
              this.recuperarCargos()
            },
            (error) => {
